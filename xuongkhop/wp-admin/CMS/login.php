<?php
	...
	{	
		...
		$sql = "SELECT * FROM bstv WHERE TenBS='$username' AND MatKhau ='$password'";
		...
		$_SESSION['tg_login_id'] = $row_user['ID_BacSi'];
		$_SESSION['tg_login_user'] = $row_user['TenBS'];
		$_SESSION['tg_login_kytu'] = $row_user['KyTuBacSi'];
		$_SESSION['SoDatHen'] = $row_user['SoDatHen'];
		$_SESSION['tg_login_level'] = $row_user['idGroup'];		

			if (strlen($_SESSION['back'])>0){
				$back = $_SESSION['back']; //unset($_SESSION['back']);
				header("location:$back");
			} else header("location: main.php?p=dh_xem");
		} else { //Thất bại
...
<div class="box">
	<div class="welcome" id="welcometitle">Chào Bác Sĩ, mời bác đăng nhập : <!--//  Welcome message -->
</div>
