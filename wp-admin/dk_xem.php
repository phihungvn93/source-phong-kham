...
?>
<?php
  $loaisp = $qt->ListDK($totalRows, $pageNum,$pageSize, $sdt,$IP);
?>
<style>
...
<p>
<a href="http://benhviennamkhoahcm.com/wp-admin/main.php?p=dk_xem"><input type="button" style="width:200px" value="Cập Nhật" /></a>
<a href="http://benhviennamkhoahcm.com/wp-admin/main.php?p=dk_xem&khongcapnhat=1"><input type="button" style="width:200px" value="Dừng Cập Nhật" /></a>
</p>
<br/>
...
<?php ob_start(); ?>
<tr class="mau{ghichu}">    <td class="anhien">{id}</td>
        <td>{NgayGioDK}</td> 
        <td>{sdt}</td>
...
<?php
$str = ob_get_clean();
$str = str_replace("{id}",$row_loaisp['id_sdt'],$str);
$str_sdt = $row_loaisp['sdt'];
...
$str = str_replace("{NgayGioDK}",date("H:i:s --- d/m/Y",strtotime($row_loaisp['NgayGioDK'])+7*3600),$str);
