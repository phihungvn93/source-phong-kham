     ...
     <td>
     	<select  id="Parent" name="Parent">
          ...
          <?php while ($row_dm = mysql_fetch_assoc($danhmuc)) { ?>
            <option value="<?php echo $row_dm['idLoai']; ?>" style="font-weight:bold"><?php echo $row_dm['TieuDe']; ?></option>
			<?php 
				$danhmuccon = $qt->DanhMuc(-1, $row_dm['idLoai']);
				while($row_dmc = mysql_fetch_assoc($danhmuccon)){
			?>
				<option value="<?php echo $row_dmc['idLoai']?>">--<?php echo $row_dmc['TieuDe']?></option>
			<?php }?>
          <?php } ?>
        </select>
     </td>
